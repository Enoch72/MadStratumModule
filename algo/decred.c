...
*/
void Black256CalcMidstate(void *in, void *out)
{
	sph_blake256_context blake_mid;
	sph_blake256_init (&blake_mid);
	sph_blake256(&blake_mid, in, 128);
	memcpy(out, blake_mid.H, 32);
}
...
void decred_hash_DCR(void *state, const void *input)
{
	ctx_midstate_done = false;
	decred_hash(state, input);
}

bool CheckDCRWork(struct work *work)
{

	uint32_t hash32[8];
	uint32_t *pdata = work->data;
	uint32_t *ptarget = work->target;
	decred_hash_simple(hash32, pdata);
	return fulltest(hash32, ptarget);
}
...
int scanhash_decred(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	*hashes_done = n - first_nonce + 1;
	...
	applog(LOG_WARNING, "Nonce first: %i   max_nonce: %i  done: %i", first_nonce, max_nonce,*hashes_done);
