	...
	if (have_stratum) {
		...
		if (jsonrpc_2) {
			...
			if (opt_algo == ALGO_DECRED) {
				unsigned int data;//Cambiata da me!
				//for (unsigned i = 0; i < 5; i++)
			//	{
			//		be32enc(&data, work->data[36 + i]);
			//		work->data[36 + i] = data;
			//	}
			...
			static unsigned int numsid = 0;

			snprintf(s, JSON_BUF_LEN,
					"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}", // prima al posto di %u c'era 4
					rpc_user, work->job_id, xnonce2str, ntimestr, noncestr, numsid++);
			free(xnonce2str);
		}
...
bool WorkDataIs0(const char *data,uint32_t const wsize)
{
...
void PrepareAndSendWorkToDCRMiner(struct work *WorkData)
{
	FILE *F = NULL;
	uint32_t counter = 200;
	do
	{   // Diamo il tempo al client di leggere
		
		if (fopen_s(&F, "DCR_to_Miner.dat", "wbT"))
		{
			Sleep(1); counter--;
		}
		else
	...
	{
		uint8_t tmpdata[180];
		uint32_t hash[8], midstate[8];
		uint32_t job_id = (int)strtol(WorkData->job_id, NULL, 16);
		fwrite(&job_id, 1, sizeof(job_id), F);
		fwrite(WorkData->data, 1, sizeof(tmpdata), F);
		fwrite(WorkData->target, 1, 32, F);
		fwrite(&WorkData->height, 1, 4, F);
		fwrite(&accepted_count, 1, 4, F);
		fwrite(&rejected_count, 1, 4, F);
		fclose(F);
		// I primi 32 bytes di data spediti alla GPU sono il midstate (0..7)
		// Gli ultimi 13 valori di uint32 (32...44) sono gli altri valori
		// -- 35 - 36 -35 nonces
	}
}

bool ReadDataFromFile(char *FileName, uint32_t datalen,uint32_t *id,void *out)
{
	// fopen("DCR_to_Miner.dat"
	bool result = false;
	FILE *F = NULL;
	F = fopen(FileName, "rb");
	if (F)
	{
		size_t readed = 0;
		readed+=fread(id, 1, sizeof(uint32_t), F);
		readed+=fread(out, 1, datalen, F);
		fclose(F);
		DeleteFile(FileName);
		result = (readed == (sizeof(uint32_t) + datalen) ) ;
	}
	return result;
	// I primi 32 bytes di data spediti alla GPU sono il midstate (0..7)
	// Gli ultimi 13 valori di uint32 (32...44) sono gli altri valori
...
uint32_t GetWorkDoneByDCRMiner(struct work *work_done)
{
	uint32_t result = 0;
	FILE *F = NULL;
	uint32_t counter = 200;
	...
	char buffer[24];
	if (fopen_s(&F,"DCR_from_Miner.dat", "rb")== 0)
	{
		uint32_t job_id,readed=0;
		readed+=fread (&job_id, 1, sizeof(job_id), F);
		readed+=fread(&work_done->data[0], 1, 180, F);
		work_done->job_id = strdup(_itoa(job_id,buffer, 16));
		fclose(F);
		DeleteFile("DCR_from_Miner.dat");
		result = 1;
	}
	return result;
}
...
static void *DCR_miner_thread(void *userdata)
{
	struct thr_info *mythr = (struct thr_info *) userdata;
	int thr_id = mythr->id;
	struct work work;
	struct work old_work;
	struct work work_done;
	...
	memset(&old_work, 0, sizeof(old_work));
	memset(&work_done, 0, sizeof(old_work));

	// Aspettiamo che lo stratum abbia ricevuto il lavoro 
	...
	while (1) {
		...
		if (stratum.job.job_id == NULL)
		 {  // Waiting for first stratum work
			Sleep(1);
			continue;
		 }
		Sleep(1);
		...
		__try
		{   // Verifica che è cambiato il job stratum!
			pthread_mutex_lock(&g_work_lock);
			if (have_stratum && ( (work.job_id==NULL) || strcmp(stratum.job.job_id, work.job_id)) ) 
			{
				work_copy(&old_work, &work);
				stratum_gen_work(&stratum, &work);
				if (work.job_id)
					free(work.job_id);
				work.job_id = NULL;
				if (stratum.job.job_id)
					work.job_id = strdup(stratum.job.job_id);
				PrepareAndSendWorkToDCRMiner(&work);
			}
			/*
			else
			{  // NO STRATUM  not (yet?) support for it 
		...
		rc = GetWorkDoneByDCRMiner(&work_done);
		
		/* if nonce found, submit work */
		if ( rc ) 
		{  // Copiare il current work
			pthread_mutex_lock(&g_work_lock);
			bool stale = strcmp(work_done.job_id, stratum.job.job_id);
			uint32_t *ptr_nonce      = ((uint32_t *)(work.data))      + 35;
			uint32_t *ptr_nonce_done = ((uint32_t *)(work_done.data)) + 35;
			
			// mi interessano i 3 bytes (35,36,37)
			
			pthread_mutex_unlock(&g_work_lock);

			if (false && stale)
				applog(LOG_NOTICE, "CURRENT JOB ID JUST CHANGED: HASH STALED, NOT SUBMITTED");
			else
			{
				memcpy(ptr_nonce, ptr_nonce_done, 3 * sizeof(uint32_t));
				if (CheckDCRWork(&work))
				{
					if (!submit_work(mythr, &work))
						break;
					applog(LOG_NOTICE, "DCR hash received from GPU: VALID.");
				}
				else
				{
					applog(LOG_ERR, "DCR hash received from GPU : INVALID");
					uint32_t *ptr_nonce = ((uint32_t *)(work.data)) + 35;
					char buffer[255];
					sprintf(buffer,"NONCES: %X - %X - %X", ptr_nonce_done[0], ptr_nonce_done[1], ptr_nonce_done[2]);
					applog(LOG_BLUE, buffer);
				}
			}
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		if (have_stratum) {
			...
			if (regen_work) {
				applog(LOG_BLUE, "REGEN_WORK BY SCANHASH");
				...
				if (stratum.job.job_id)
					work.job_id  = strdup(stratum.job.job_id);
				applog(LOG_BLUE, "nonceptr = %i",*nonceptr);
				*nonceptr = 0;
		...
		if (opt_algo != ALGO_SIA && have_stratum && WorkDataIs0((const char *)work.data,sizeof(work.data)) && !opt_benchmark) {
			sleep(10);
			continue;
		}
		...
		case ALGO_CRYPTONIGHT:
			...
			break;
		case ALGO_DECRED:
			applog(LOG_NOTICE, "SCANHASH_ENTER");
			rc = scanhash_decred(thr_id, &work, max_nonce, &hashes_done);
			applog(LOG_NOTICE, "SCANHASH_EXIT");
			break;
		case ALGO_DROP:
...
static void *stratum_thread(void *userdata)
{
	...
	while (1) {
		...
		{
			applog(LOG_BLUE, "NEW_WORK FROM STRATUM SERVER");
			pthread_mutex_lock(&g_work_lock);
			stratum_gen_work(&stratum, &g_work);
			time(&g_work_time);
			pthread_mutex_unlock(&g_work_lock);
...
{
	printf("==========================================\n=");
	applog(LOG_WARNING, "StratumModule for MadMiner DCR only.\n");
	printf("==========================================\n=");
	printf("Derived from : \n");
	printf(" cpuminer-multi by tpruvot@github **\n");
	printf("Source code of the module available at\n");
	printf("https://github.com/Enoch72/MadStratumModule");
}
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < opt_n_threads; i++) {
		...
		err = thread_create(thr, DCR_miner_thread);
		if (err) {
